...
void PeriodFinderTest(){
    lli N = 21;
    lli a = 11;
    bool runtillfindingperiod = false;
    std::pair<lli,lli> p = {-1,-1};
    int counter = 0;
    time_point<system_clock> start, end;
    start = system_clock::now();
    if(runtillfindingperiod){
    do{
        auto* dd = new dd::Package;
    ...
    printf("period of %lld mod %lld found: %lld in %d runs and %f seconds\n",a,N,p.second, counter, elapsed_seconds.count());
        ...
        duration<float> elapsed_seconds = end - start;
        if((p.second>0) && shor::modexp(a,p.second,N) == 1)
            printf("period of %lld mod %lld: found %lld in %f seconds\n",a,N,p.second, elapsed_seconds.count());
        else printf("period of %lld mod %lld: failed in %f seconds\n",a,N, elapsed_seconds.count());
    }
}
void PeriodFinderAverageRuntime(){
    lli N = 21;
