...
SB_ProbabilisticPeriodFinder::SB_ProbabilisticPeriodFinder(lli N, lli a, dd::Package *dd) : gg(dd), posControl(1), m_qft(dd)
{
    ...
    m_base2N = [base2N](int i)
    { return base2N.empty() ? 0 : base2N[i]; };
...
{
    for (int i = n; i >= 0; --i)
    { //plus one to accomodate for overflow
        for (int j = i; j >= 0; --j)
        {
...
void SB_ProbabilisticPeriodFinder::phi_Subtractor(const std::function<int(int)> &acnum, const map<int, bool> &c, short *line, dd::Edge &state)
{
    ...
    for (int i = 0; i < n + 1; ++i)
    { //plus 1 to accomodate for overflow.
        for (int j = 0; j <= i; ++j)
        {
            if (acnum(j) == posControl)
            {
...
void SB_ProbabilisticPeriodFinder::phi_CMultiplier(const lli &a0cnum, const std::function<int()> &tindex, map<int, bool> c, short *line, dd::Edge &state)
{
    ...
    m_qft.dd_QFTV5(nt, state, m_qftindices, false); //qft
    for (int i = 0; i < n; ++i)
    { //fig6 of paper, repeated shift and apply
        ...
        auto a0cnumbase2 = [&a0clb2](int i)
        { return ((a0clb2.empty() || i >= a0clb2.size()) ? 0 : a0clb2[i]); }; //return cnum's digits in base 2.
...
void SB_ProbabilisticPeriodFinder::phi_CDivider(const lli &acnum, const std::function<int()> &tindex, map<int, bool> c, short *line, dd::Edge &state)
{
    ...
    for (int i = n - 1; i >= 0; --i)
    { //fig5 of paper, repeated shift and apply
        //TODO: make next four opertations not do redundant calculations.
...
/// Returns index of currently 'read' (by the code, it is not measurenemt)
/// qubit taking into account the swap operation between b and x qubits
/// in c_Ua gates. See fig 7 of SB.
/// @param i i-th qubit to be read, from qubits currently carrying information
/// of of 'exponentiation'. See fig 6 of SB, 'b' qubits.
/// @return actual index of i-th qubit.
