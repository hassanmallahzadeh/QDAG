...
/// @param state input quantum state
/// @param xindice  quantum register to be multiplied by acnum mod N. x indice (for input quantum number)
void SB_PPF::out_Circuit(const lli &acnum, const std::function<int (int)> &bindice, const std::function<int (int)> &xindice, const std::function<int ()> &tindex, map<int,bool> c, short *line, dd::Edge &state){
    for(int i = 0 ; i < 2 * n; ++i){
     //   phi_CUa()
    }
    phi_CMultiplier(acnum,bindice,xindice,tindex,c, line, state);
