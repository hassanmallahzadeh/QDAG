...
dd::Edge RegisterFactory::RippleAdderHalfClassicDebug(lli cnum, vector<int> num){
    ...
    ExtractedRippleAdderHalfClassicV1(b0indice, c0indice, c1indice, a0clb2, line, n, nt, state);
    delete[] line;
    return state;
}
void RegisterFactory::ExtractedRippleAdderHalfClassicV1(const std::function<int (int)> &b0indice, const std::function<int (int)> &c0indice, const std::function<int (int)> &c1indice, vector<bool> a0clb2, short *line, int n, int nt, dd::Edge &state) {
    ...
    //execute ripple adder:
    for (int i = 0; i < n; ++i){
        lambdaCarry(c0indice(i), a0clb2[i], b0indice(i), c1indice(i));
    }
    if(a0clb2[n-1])
        gg.NotGenOrApply(line, b0indice(n-1), nt, &state);
 
    lambdaSum(c0indice(n-1), a0clb2[n-1], b0indice(n-1));
    for (int i = n - 2; i >= 0; --i){
        lambdaCarryInv(c0indice(i), a0clb2[i], b0indice(i), c1indice(i));
        lambdaSum(c0indice(i), a0clb2[i], b0indice(i));
    }
}
///// Like the other version, just vector<bool> a0clb2 is replaced by t which means qubit t negative controls gates that a0clb2 digits controlled classically
void RegisterFactory::ExtractedRippleAdderHalfClassicV2(const std::function<int (int)> &b0indice, const std::function<int (int)> &c0indice, const std::function<int (int)> &c1indice, int t, short *line, int n, int nt, dd::Edge &state){
    auto lambdaCarry = [&state, this, line, nt, t](int c0, int b0, int c1){
        gg.ToffoliGenOrApply(line, c1, b0, t, nt, &state, true, false);
        gg.CNotGenOrApply(line, b0, nt, t, &state, false);
        gg.ToffoliGenOrApply(line, c1, c0, b0, nt, &state);
    };
    auto lambdaCarryInv = [&state, this, line, t, nt](int c0, int b0, int c1){
        gg.ToffoliGenOrApply(line, c1, c0, b0, nt, &state);
        gg.CNotGenOrApply(line, b0, t, nt, &state, false);
        gg.ToffoliGenOrApply(line, c1, b0, t, nt, &state,true , false);
    };
    auto lambdaSum = [&state, this, line, t, nt](int c0, int b0){
        gg.CNotGenOrApply(line, b0, c0, nt, &state);
        gg.CNotGenOrApply(line, b0, t, nt, &state, false);
    };
    //execute ripple adder:
    for (int i = 0; i < n; ++i){
        lambdaCarry(c0indice(i), b0indice(i), c1indice(i));
    }
    gg.CNotGenOrApply(line, b0indice(n-1), t, nt, &state, false);
    lambdaSum(c0indice(n-1), b0indice(n-1));
    for (int i = n - 2; i >= 0; --i){
        lambdaCarryInv(c0indice(i), b0indice(i), c1indice(i));
        lambdaSum(c0indice(i), b0indice(i));
    }
}
...
/// @param n base 2 for storing initial quantum number.
...
dd::Edge RegisterFactory::ModuloNAdderDebug(vector<int> num1, vector<int> num2){
    assert(num1.size() == num2.size());
    std::reverse(num1.begin(),num1.end());//must significant number is at index 0 in code below.
...
dd::Edge RegisterFactory::ModuloNAdderHalfClassicDebug(lli cnum, vector<int> qnum){
    ...
    ExtractedRippleAdderHalfClassicV1(b0indice, c0indice, c1indice, a0clb2, line, n, nt, state);
    ExtractedRippleSubtractorHalfClassic(base2N, b0indice, c0indice, c1indice, line, nt, state);
    gg.NotGenOrApply(line, c1indice(n - 1)/*nt - 2*/, nt, &state);
    gg.CNotGenOrApply(line, tindice(), c1indice(n - 1)/*nt - 2*/, nt);
    gg.NotGenOrApply(line, c1indice(n - 1)/*nt - 2*/, nt, &state);
    ExtractedRippleAdderHalfClassicV2(b0indice, c0indice, c1indice, tindice(), line, n, nt, state);
    ExtractedRippleSubtractorHalfClassic(a0clb2, b0indice, c0indice, c1indice, line, nt, state);
    gg.CNotGenOrApply(line, tindice(), c1indice(n - 1), nt);
    ExtractedRippleAdderHalfClassicV1(b0indice, c0indice, c1indice, a0clb2, line, n, nt, state);
    delete[] line;
    return state;
}
void RegisterFactory::ExtractedRippleSubtractorHalfClassic(vector<bool> &a0clb2, const std::function<int (int)> &b0indice, const std::function<int (int)> &c0indice, const std::function<int (int)> &c1indice, short *line, int nt, dd::Edge &state) {
    auto lambdaCarry = [&state, this, line, nt](int c0, int a0, int b0, int c1){
        if(a0){
            gg.CNotGenOrApply(line, c1, b0, nt, &state);
            gg.NotGenOrApply(line, b0, nt, &state);
