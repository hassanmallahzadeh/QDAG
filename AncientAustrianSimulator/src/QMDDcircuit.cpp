...
QMDDedge QMDDreadGateFromString(char *str, QMDDrevlibDescription *circ)
{
	...
	else
	{
		...
		{
			mpfr_tmp1 = QMDDcos(1, div);
			mpfr_tmp2 = QMDDsin(1, div);
			Qm[1][1]=Cmake(mpfr_tmp1,mpfr_tmp2);
		...
		else if(ch1=='R') // Rotation gate
		{
			...
			if(ch2=='X')
			{
				mpfr_tmp1 = QMDDcos(m, div);
				mpfr_tmp2 = 0;
				//mpfr_set_si(mpfr_tmp2, 0, MPFR_RNDN);

				Rm[0][0]=Rm[1][1]=Cmake(mpfr_tmp1,mpfr_tmp2);
				mpfr_tmp1 = QMDDsin(m, -div);
				Rm[0][1]=Rm[1][0]=Cmake(mpfr_tmp2,mpfr_tmp2);
			} else if(ch2=='Y')
			{
				mpfr_tmp1 = QMDDcos(m, div);
				mpfr_tmp2 = 0;
				...
				Rm[0][0]=Rm[1][1]=Cmake(mpfr_tmp1,mpfr_tmp2);
				mpfr_tmp1 = QMDDsin(m, -div);
				Rm[0][1]=Cmake(mpfr_tmp1,mpfr_tmp2);
				mpfr_tmp1 = QMDDsin(m, div);
				Rm[1][0]=Cmake(mpfr_tmp1,mpfr_tmp2);
			} else if(ch2=='Z')
			{
				mpfr_tmp1 = QMDDcos(m, div);
				mpfr_tmp2 = QMDDsin(m, -div);

				Rm[0][0]=Cmake(mpfr_tmp1,mpfr_tmp2);
				mpfr_tmp2 = 0;
				//mpfr_set_si(mpfr_tmp2, 0, MPFR_RNDN);
				Rm[0][1]=Rm[1][0]=Cmake(mpfr_tmp2, mpfr_tmp2);
				mpfr_tmp2 = QMDDsin(m, div);
				Rm[1][1]=Cmake(mpfr_tmp1,mpfr_tmp2);
			} else {
	...
	//TODO might be inefficiently to clear here becasue its inside a loop

	//mpfr_clear(mpfr_tmp1);
	//mpfr_clear(mpfr_tmp2);
