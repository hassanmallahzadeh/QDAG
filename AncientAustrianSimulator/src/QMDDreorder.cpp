...
QMDDedge QMDDrenormalize(QMDDedge a) {
 ...
 QMDDresetVertexWeights(a, 0);
 QMDDresetVertexWeights(a, 1);
...
// make a QMDD nonterminal node using existing space pointed to by p
  ...
  if(e.w != 1) {
   ...
   if (debugSift) { printf("Debug: adjusting renormalization factor of node %d. From ", (intptr_t) e.p); Cprint(Cvalue(e.p->renormFactor));}
    ...
    if (e.p->renormFactor == 1) // renormFactor is sure to become non-trivial
      RenormalizationNodeCount++;
      
    e.p->renormFactor = Cmul(e.p->renormFactor, e.w);
    if (debugSift) { printf(" to ");  Cprint(Cvalue(e.p->renormFactor)); printf("\n");}
    e.w=1;
